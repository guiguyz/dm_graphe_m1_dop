...
public class Arbre {
    ...
    private final Matrice<Boolean> matrice;

    // Matrice[i,j] qui indique la distance non ponderee entre la ville i et j
    private final Matrice<Double> distance;
     ...
     * Contruit un 1-arbre
     *
     * @param villes Tableau des villes (trie par id)
     */
    public Arbre(Ville[] villes) {
        this.villes = villes;

        // Xij indique si l'arc (i,j) est actif ou non
        matrice = new Matrice<Boolean>(villes);

        // Stoque les distances entre les villes
        distance = new Matrice<Double>(villes);
    ...
    public boolean arcActif(int i, int j) {
        return matrice.get(i, j) != null && matrice.get(i, j) == true;
    }
     ...
     *
     * @param i Numero du sommet 0 a n-1
     * @return Le degre
     */
     ...
     * 0 avant le 1e appel
     *
     * @param nbIgnore Indique les N premieres villes a ignorer
     */
    public void prim() {
